...
#     Bug 4 fix: returns None for labels that are neither in _LABEL_ALIASES
#     nor in settings.ALLOWED_NODES after title-casing, so hallucinated LLM
#     labels are rejected at storage time rather than silently polluting the graph.
#     """
#     # Step 1: check alias dict
#     canonical = _LABEL_ALIASES.get(label.lower())
#     if canonical:
#         return canonical
#     # Step 2: apply title-case and check against ALLOWED_NODES whitelist
#     titled = label.title()
#     if titled in settings.ALLOWED_NODES:
#         return titled
...
#                     {},
#                 )
#                 valid_rows.append({"from_id": from_id, "to_id": to_id, "props": matched_props})
