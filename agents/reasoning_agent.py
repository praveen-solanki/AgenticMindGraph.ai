...
def run(
    ...
    impacted_node_ids: list[str] | None = None,
) -> ReasoningResult:
    """
    ...
    Args:
        ...
        impacted_node_ids:  Node IDs from the Impact Agent (IMPACT_OF edge targets).
                            These are fetched from Neo4j and injected into the LLM
                            context so IMPACT_OF edges are not a write-only artefact.
    ...
    try:
        ...
        # ── 3b. Fetch and inject impacted nodes (IMPACT_OF consumers) ─────────
        impact_context = ""
        if impacted_node_ids:
            impact_context = _fetch_impacted_nodes_context(neo, impacted_node_ids, result)

        # ── 4. Assemble context string (token-capped) ─────────────────────────
        context_str = _build_context(seed_chunks, subgraph, impact_context)
...
def _embed_question(question: str) -> list[float] | None:
    """Embed the question using the same model as ingestion (cached singleton)."""
    try:
...
# Module-level cached embedding model to avoid reloading ~2GB per query
...
def _get_embed_model():
    ...
    if _embed_model_instance is None:
        from sentence_transformers import SentenceTransformer
        _embed_model_instance = SentenceTransformer(settings.EMBED_MODEL)
...
def _expand_subgraph(
    ...
    # Traverse: chunk → mentions → entity/requirement/concept/gap → follow relations
    # POLICY: Prevent "Super-Node Explosion" by blocking traversal THROUGH Document nodes.
    # A Document node can be an endpoint (the citation source), but never an 
    # intermediate bridge between two unrelated concepts.
    cypher = f"""
    UNWIND $chunk_ids AS cid
    MATCH path = (start:Chunk {{id: cid}})-[*1..{max_hops}]-(related)
    WHERE any(lbl IN labels(related) WHERE lbl IN $allowed_labels)
      AND related.name IS NOT NULL
      AND NONE(n IN nodes(path)[1..-1] WHERE 'Document' IN labels(n))
    RETURN
    ...
    # Include all semantic nodes plus Document (but exclude Chunk to avoid returning the chunks themselves)
    allowed_labels = [lbl for lbl in settings.ALLOWED_NODES if lbl != "Chunk"] + ["Document", "SpecGap"]

    try:
        rows = neo.run(cypher, chunk_ids=chunk_ids, node_limit=node_limit, allowed_labels=allowed_labels)
        for r in rows:
            node_entry = {
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 3b — Impacted node context (IMPACT_OF consumers)
# ══════════════════════════════════════════════════════════════════════════════

def _fetch_impacted_nodes_context(
    neo: Neo4jClient,
    impacted_node_ids: list[str],
    result: ReasoningResult,
) -> str:
    """
    Fetch the nodes that are targets of IMPACT_OF edges (written by the Impact
    Agent) and return a formatted context block for the LLM.

    Without this, IMPACT_OF edges are a write-only artefact — the Reasoning
    Agent never sees which nodes were flagged as impacted.
    """
    ...
    RETURN n.id   AS node_id,
           ...
           coalesce(n.definition, n.description, n.text, n.raw_text, '') AS node_desc
    LIMIT 50
    """
    try:
        rows = neo.run(cypher, node_ids=impacted_node_ids)
    except Exception as exc:
        result.errors.append(f"Impacted node fetch failed: {exc}")
        return ""

    if not rows:
        return ""

    lines = ["=== Impact Analysis — Nodes Flagged by Impact Agent (via IMPACT_OF edges) ==="]
    for r in rows:
        entry = f"[{r.get('node_label', 'Node')}] {r.get('node_name', r.get('node_id', '?'))}"
        desc  = str(r.get("node_desc", "")).strip()
        if desc:
            entry += f"\n  → {desc[:300]}"
        lines.append(entry)

    log.info("  Reasoning: injected %d impacted nodes from Impact Agent", len(rows))
    return "\n\n".join(lines)
...
def _build_context(
    ...
    + impacted nodes from the Impact Agent.
    ...
    # Impacted nodes from Impact Agent (IMPACT_OF edge targets)
    if impact_context:
        parts.append(f"\n{impact_context}")
