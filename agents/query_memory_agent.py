...
def run(
    ...
    try:
        ...
        _prune_old_patterns(neo, report)
...
def _store_pattern(neo: Neo4jClient, result: dict, report: QueryMemoryReport) -> None:
    ...
    classification = call_agent_llm_json(
        "fast_classify", _CLASSIFY_SYSTEM, f"Question: {question}", max_tokens=1024
    )
    category    = "general"
    entities    = []
    complexity  = "simple"
...
def _prune_old_patterns(neo: Neo4jClient, report: QueryMemoryReport) -> None:
    """
    Delete QueryPattern nodes that have not been asked recently and are
    not hot spots. Keeps the label bounded and limits sensitive data retention.

    Retention policy: keep nodes asked within last ASEI_QUERY_MEMORY_RETENTION_DAYS
    days OR asked >= ASEI_QUERY_MEMORY_HOT_SPOT_COUNT times (hot spots kept forever).
    """
    q = """
    MATCH (qp:QueryPattern)
    WHERE qp.ask_count < $hot_threshold
      AND qp.last_asked_at < $cutoff
    WITH qp LIMIT $batch
    DETACH DELETE qp
    ...
    cutoff = (
        ...
        - timedelta(days=settings.ASEI_QUERY_MEMORY_RETENTION_DAYS)
    ).isoformat()
    try:
        rows = neo.run(
            q,
            hot_threshold=settings.ASEI_QUERY_MEMORY_HOT_SPOT_COUNT,
            cutoff=cutoff,
            batch=200,
        )
        deleted = rows[0]["deleted"] if rows else 0
        if deleted:
            log.info("  QueryPattern pruned: %d old nodes deleted", deleted)
    except Exception as exc:
        report.errors.append(f"QueryPattern pruning failed: {exc}")
